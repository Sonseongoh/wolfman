...
public class AttackPowerRuleTests
{
    ...
    {
        Assert.That(AttackPowerRule.Damage(10, 0f, 1f, 1f), Is.EqualTo(10));
    }
    ...
    public void 스킬_공격력_보너스는_합연산이다()
    {
        ...
        Assert.That(AttackPowerRule.Damage(100, 0.30f, 1f, 1f), Is.EqualTo(130));
    }

    [Test]
    public void 달_배율이_곱해진다()
    {
        Assert.That(AttackPowerRule.Damage(10, 0f, 1.5f, 1f), Is.EqualTo(15));
    }
    ...
    {
        // 100 × 1.2 × 1.5
        Assert.That(AttackPowerRule.Damage(100, 0.2f, 1.5f, 1f), Is.EqualTo(180));
    }
    ...
    public void 아무리_깎여도_최소_1은_들어간다()
    {
        ...
        Assert.That(AttackPowerRule.Damage(1, 0f, 0.1f, 1f), Is.EqualTo(1));
        Assert.That(AttackPowerRule.Damage(10, -1f, 1f, 1f), Is.EqualTo(1));
    }
    ...
    public void 정확히_반이면_짝수쪽으로_붙는다(int baseDamage, float moon, int expected)
    {
        ...
        Assert.That(AttackPowerRule.Damage(baseDamage, 0f, moon, 1f), Is.EqualTo(expected));
    }
    ...
    public void 기본_데미지가_작으면_배율이_계단으로_뭉갠다()
    {
        ...
        Assert.That(AttackPowerRule.Damage(5, 0f, 0.85f, 1f), Is.EqualTo(4));
        Assert.That(AttackPowerRule.Damage(5, 0f, 0.70f, 1f), Is.EqualTo(4));
        Assert.That(AttackPowerRule.Damage(5, 0f, 0.50f, 1f), Is.EqualTo(2));
    }
    ...
    // "굶주림 배율 1 이면 기존 식과 같다"는 테스트가 여기 있었다. 굶주림 없이 계산하는
    // 3-인자 오버로드를 없애면서 자기 자신과 비교하는 항등식이 되어 지웠다.
    // 옛 식과 같은가는 유니티(Mono)의 AttackPowerMonoChecks 고정 4건이 지킨다.
    ...
    public void 발톱과_참격은_같은_식을_지난다()
    {
        ...
        int claw = AttackPowerRule.Damage(10, skill, moon, 1f);   // 발톱 base 10
        int wave = AttackPowerRule.Damage(5, skill, moon, 1f);    // 달빛 참격 base 5
